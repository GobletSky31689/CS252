...
exprP' = do
  ...
  return (case rest of
    ...
    Just ("*", t') -> Op Times t t'
    Just ("/", t') -> Op Divide t t'
    Just (op, t') -> isMultiplyDivide op t t')


isMultiplyDivide prevOp prevPredicate predicate = do
  spaces
  x <- optionMaybe isMultiplyDivide'
  spaces
  return (case x of 
    Nothing -> Op (transOp prevOp) prevPredicate predicate
    Just (op, e) -> Op (transOp prevOp) prevPredicate (Op (transOp op) predicate (isMultiplyDivide op predicate e)))
